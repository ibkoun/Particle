...
def quadratic(a, b, c):
    ...
    if math.pow(b, 2) - 4 * a * c < 0:
        print("No solution")
    # discriminant = math.sqrt(discriminant) if discriminant > 0 else math.sqrt(b, 2)
...
def point_on_circumference(radius, center, start, end):
    ...
    # bx = 2 * start[0] * (end[0] - start[0]) - 2 * center[0] * (end[0] - start[0])
    bx = -2 * (end[0] - start[0]) * (center[0] - start[0])
    # by = 2 * start[1] * (end[1] - start[1]) - 2 * center[1] * (end[1] - start[1])
    by = -2 * (end[1] - start[1]) * (center[1] - start[1])
    b = bx + by

    # cx = math.pow(start[0], 2) - 2 * start[0] * center[0] + math.pow(center[0], 2)
    cx = math.pow((center[0] - start[0]), 2)
    # cy = math.pow(start[1], 2) - 2 * start[1] * center[1] + math.pow(center[1], 2)
    ...
    delta1 = end - p1
    delta2 = end - p2
    d1 = np.dot(delta1, delta1)
    d2 = np.dot(delta2, delta2)
